...
var ref$, mkRealtimeInput, mkRadiobox, weblibTest, stateOf, radioboxHandler, radioboxListenerHandler, connectEnterToClick, flatten, initial, drop, join, concat, tail, head, map, zip, split, union, url, arr, addr_port, socketioPath, socket, ActorBase, ActorManager, Actor, ProxyActor, SwitchActor, app, setSwitchButtons, setPushButtons, setStatusLeds, getRactiveVariable, setRactiveVariable, setAnalogDisplays;
...
setAnalogDisplays = function(){
  return $('.analog-display').each(function(){
    ...
    jqElem = $(this);
    channelName = getRactiveVariable(jqElem, 'channel');
    console.log("this is channel name: ", channelName);
    actor = SwitchActor(channelName);
    return actor.addListener(function(msg){
      return setRactiveVariable(jqElem, 'value', msg.val);
    });
  });
};
